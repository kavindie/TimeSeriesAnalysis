...
class CouplingNSF(nn.Module):
    ...
    def __init__(self, dim, dimc=0, K=5, B=3, hidden_dim=8, device=0):
        super().__init__()
        self.dim = dim
        self.dimc = dimc  # conditionals dim : image+past
    ...
    def _get_spline_params(self, x1, c=None):
        x = x1 if c is None else torch.cat((x1, c), -1)  # concat inputs
        out = self.conditioner(x).view(-1, self.dim - (self.dim // 2),
                                       3 * self.K - 1)  # calls f(x_1:d), arange spline params by input dim
    ...
    def forward(self, x, c=None):
        if self.dim == 1:
            ...
            x, ld = unconstrained_RQS(x, W, H, D, inverse=False, tail_bound=self.B)
        else:
            # compute spline parameters
    ...
    def inverse(self, z, c=None):
        ...
        if self.dim == 1:
            W, H, D = self._get_spline_params(c)
            # apply spline transform
            z, ld = unconstrained_RQS(z, W, H, D, inverse=True, tail_bound=self.B)
        else:
...
class NeuralSplineFlow(nn.Module):
    ...
    def __init__(self, nin, nc=0, n_layers=5, K=5, B=3, hidden_dim=8, device=0):
        ...
        self.net = []
        reverse_only = (self.nin == 2)
        for i in range(self.n_layers):
            self.net.append(CouplingNSF(self.nin, self.nc, K, B, hidden_dim=hidden_dim, device=device))
            self.net.append(InvertiblePermutation(self.nin, reverse_only=reverse_only))
        self.net.pop()
