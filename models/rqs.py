...
def unconstrained_RQS(inputs, unnormalized_widths, unnormalized_heights,
    ...
    if inside_intvl_mask.any():
        ...
        print(inputs)
        print(f'Found a case where none of the points were inside the range')
...
def RQS(inputs, unnormalized_widths, unnormalized_heights,
    ...
    if inverse: # compute the inverse spline transformation
        ...
        negdisc = (discriminant < 0)
        if negdisc.any():
            print(f"Negative discriminant(s) found ({negdisc.sum()})", file=stderr)
            # Stefan: set discriminants to 0 as closest valid answer
            discriminant[negdisc] = 0
